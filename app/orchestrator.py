...
from services.upsert import upsert_embeddings  # 👈 NEW
...
class ClinicalOrchestrator:
    ...
    def run_clinical_nlp(self, state: ClinicalState) -> ClinicalState:
        ...
        for doc in state.raw_documents:
            try:
                ...
                state.add_error(f"NLP failed for {doc.get('source')}: {str(e)}")
    ...
    def run_embedding(self, state: ClinicalState) -> ClinicalState:
        ...
        if not state.nlp_results:
            state.add_error("No NLP results for embedding")
            return state
        ...
        for item in state.nlp_results:
            try:
                ...
                state.add_error(f"Embedding failed for {item.get('source')}: {str(e)}")

        return state

    # -----------------------------
    # STEP 4: VECTOR UPSERT ✅ NEW
    # -----------------------------
